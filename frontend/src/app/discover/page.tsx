...
import { DISCOVERED_STATUS_LABEL, formatDate, scoreColorClass } from "@/lib/format";
import type { DiscoveryRun, OpportunityItem, SearchProfile } from "@/lib/types";
...
export default function DiscoverPage() {
  ...
  async function loadAll() {
    ...
    setError(null);
    try {
      const [opp, profs] = await Promise.all([
        api.listOpportunities({ sortBy, includeRejected: showRejected }),
  ...
  useEffect(() => {
    // eslint-disable-next-line react-hooks/set-state-in-effect -- intentional: reload feed whenever sort/filter changes
    void loadAll();
  ...
  return (
    <div className="space-y-6">
      ...
      {lastRun && (
        <Card>
          <SectionHeading title="Last run summary" />
        ...
        {!loading && opportunities.length > 0 && (
          <div className="space-y-3">
            {opportunities.map((item) => (
              <OpportunityCard key={item.discovered_job_id} item={item} />
            ))}
          </div>
        )}
      </Card>
